 ...
 main( void )
{
    ...
    while(TRUE)
    {
	    while((Rcv & I2S2_IR) == 0);  // Wait for interrupt pending flag
	    LEFT_DATA = I2S2_W0_MSW_R;  // 16 bit left channel received audio data
	    RIGHT_DATA = I2S2_W1_MSW_R;  // 16 bit right channel received audio data
	        
	    while((Xmit & I2S2_IR) == 0);
	    I2S2_W0_MSW_W = LEFT_DATA;  // 16 bit left channel transmit audio data
	    I2S2_W1_MSW_W = RIGHT_DATA; // 16 bit right channel transmit audio data
